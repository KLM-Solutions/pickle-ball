...
export default function AICoachModal({ isOpen, onClose, analyticsData }: AICoachModalProps) {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div className="relative w-full max-w-lg bg-[#0F0F12] border border-white/10 rounded-3xl shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
                {/* Header */}
                <div className="p-6 pb-2 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-indigo-300">Powered by StrikeSense AI</p>
                        </div>
                    </div>
                ...
                <div className="p-6">
                    {loading ? (
                        <div className="flex flex-col items-center justify-center py-10 space-y-4">
                            ...
                            <p className="text-indigo-300 animate-pulse font-medium">Analyzing your biomechanics...</p>
                        </div>
                    ) : error ? (
                        ...
                        <div className="space-y-6 animate-in slide-in-from-bottom-4 duration-500">
                            ...
                            <div className="bg-gradient-to-br from-indigo-500/10 to-purple-500/10 border border-indigo-500/20 rounded-xl p-4">
                                <p className="text-indigo-100 leading-relaxed italic">"{result.summary}"</p>
                            </div>

                            {/* Takeaways */}
                            <div className="space-y-3">
                                <h4 className="text-sm font-semibold text-white/80 uppercase tracking-wider flex items-center gap-2">
                                    <Sparkles className="w-4 h-4 text-emerald-400" /> Key Takeaways
                                </h4>
                                <ul className="space-y-2">
                                    {result.takeaways.map((point, i) => (
                                        <li key={i} className="flex gap-3 text-sm text-neutral-300">
                                            <span className="shrink-0 w-1.5 h-1.5 mt-2 rounded-full bg-indigo-500" />
                                            {point}
                                        </li>
                                    ))}
                                </ul>
                            ...
                            {/* Focus Drill */}
                            <div className="bg-[#1A1A1E] border-l-4 border-emerald-500 rounded-r-xl p-4">
                                <h4 className="text-emerald-400 font-bold text-sm mb-1 uppercase tracking-wide">
                                    Next Focus Drill
                                </h4>
                                <p className="text-white font-medium">{result.focus_drill}</p>
                            </div>
                        </div>
                    ) : null}
                </div>
