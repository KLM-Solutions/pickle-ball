...
export default function HistoryPage() {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case "completed":
        return (
          <span className="flex items-center gap-1 px-2 py-1 bg-white text-black rounded-full text-[10px] md:text-xs font-bold">
      ...
      case "processing":
        return (
          <span className="flex items-center gap-1 px-2 py-1 bg-neutral-800 text-neutral-300 rounded-full text-[10px] md:text-xs font-bold">
            <Loader2 className="w-2.5 h-2.5 md:w-3 md:h-3 animate-spin" /> Processing
          </span>
        );
      case "failed":
        return (
          <span className="flex items-center gap-1 px-2 py-1 bg-neutral-800 text-neutral-400 rounded-full text-[10px] md:text-xs font-bold">
            <XCircle className="w-2.5 h-2.5 md:w-3 md:h-3" /> Failed
          </span>
        );
      default:
        return (
          <span className="flex items-center gap-1 px-2 py-1 bg-neutral-800 text-neutral-500 rounded-full text-[10px] md:text-xs font-bold">
  ...
  return (
    <div className="min-h-screen bg-black text-white">
      {/* Header */}
      <header className="relative z-20 border-b border-neutral-800 sticky top-0 bg-black">
        <div className="max-w-5xl mx-auto px-4 py-3 md:py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              disabled={loading}
              className="flex items-center gap-1.5 px-2.5 md:px-3 py-2 bg-neutral-900 border border-neutral-800 rounded-lg hover:bg-neutral-800 transition text-xs md:text-sm"
            >
            ...
            <button
              onClick={() => router.push("/")}
              className="flex items-center gap-1.5 px-2.5 md:px-3 py-2 bg-white text-black rounded-lg text-xs md:text-sm font-bold"
            >
      ...
      <main className="relative z-10 max-w-5xl mx-auto px-4 py-6 md:py-8">
        ...
        {loading && (
          <div className="flex flex-col items-center justify-center py-16 md:py-20">
            <Loader2 className="w-10 h-10 md:w-12 md:h-12 text-white animate-spin mb-4" />
        ...
        {error && !loading && (
          <div className="text-center py-16 md:py-20">
            <div className="w-14 h-14 md:w-16 md:h-16 mx-auto mb-4 bg-neutral-900 rounded-full flex items-center justify-center">
            ...
            <button
              onClick={fetchHistory}
              className="px-5 md:px-6 py-2.5 md:py-3 bg-white text-black rounded-xl font-bold text-sm md:text-base"
            >
              Try Again
            </button>
          </div>
        ...
        {!loading && !error && analyses.length === 0 && (
          <div className="text-center py-16 md:py-20">
            <div className="w-16 h-16 md:w-20 md:h-20 mx-auto mb-5 md:mb-6 bg-neutral-900 rounded-full flex items-center justify-center border border-neutral-800">
              <BarChart3 className="w-8 h-8 md:w-10 md:h-10 text-neutral-600" />
            </div>
            ...
            {analyses.map((job) => {
              ...
              return (
                <div
                  ...
                  className={`bg-neutral-900 border border-neutral-800 rounded-xl md:rounded-2xl p-4 md:p-5 transition-all
                    ${job.status === "completed" && job.result_json 
                      ? "hover:bg-neutral-800 hover:border-neutral-700 cursor-pointer active:scale-[0.99]" 
                      : ""
                    }`}
                >
                  <div className="flex items-start gap-3 md:gap-4">
                    {/* Icon */}
                    <div className="w-11 h-11 md:w-14 md:h-14 rounded-xl bg-white flex items-center justify-center text-xl md:text-2xl flex-shrink-0">
                      {strokeInfo.icon}
                    </div>
                    ...
                    {job.status === "completed" && job.result_json && (
                      <button
                        ...
                        className="hidden md:flex items-center gap-2 px-4 py-2 bg-white text-black rounded-xl font-bold text-sm"
                      >
